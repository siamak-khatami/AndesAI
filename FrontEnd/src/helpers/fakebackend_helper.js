...
// User Activation
export const postUserActivation = (token) =>{
  setAuthorization(token)
  return api.create(url.USER_ACTIVATION);
}

export const postResendUserActivation = (data) =>{
  return api.create(url.RESEND_USER_ACTIVATION, data);
}
...
export const patchResetPassword = (data) =>{
  ...
  return api.update(url.RESET_PASSWORD, pass);
}

// Login Method
export const postFakeLogin = (data) => api.create(url.POST_LOGIN, data);
export const checkLogin = () => api.create(url.CHECK_LOGIN);

// Tours 
export const callDeactivateTour = (tour_id) => {
  return api.get(`${url.User_Tour}`, { params: { tour_id } } )
}


export const callGetLLMModels = () => {
  return api.get(`${url.LLMModels}`)
}
// postForgetPwd
